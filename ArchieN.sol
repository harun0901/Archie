...
/// @title ArchieInu-NFT
contract ArchieN is Ownable, ERC721Enumerable {
    ...
    uint256 public tokenAmountNFT;
    IERC20 public ERC20TokenAddress;
    address public uniswapV2Pair;
    IERC20 public WETH = IERC20(0x0a180A76e4466bF68A7F86fB029BEd3cCcFaAac5);

    constructor (string memory _pendingURI, uint256 _mintStartTime) ERC721("ArchieN", "ARCHIEN") {
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        uint256 tokenAmountPerNFTLP = cost.mul(_getRateTokenAndETH()).div(10 ** 18);
        uint256 tokenAmount;
        if (tokenAmountPerNFTLP > 0) {
            tokenAmount = tokenAmountPerNFTLP * _mintAmount;
        } else {
            tokenAmount = tokenAmountNFT * _mintAmount;
        }

        if (msg.sender != owner()) {
            require(ERC20TokenAddress.balanceOf(msg.sender) > tokenAmount, "caller have not sufficient amount");
        ...
        ERC20TokenAddress.transferFrom(msg.sender, address(this), tokenAmount);
    ...
    function _getRateTokenAndETH() public view returns(uint256){
        uint256 amountToken = ERC20TokenAddress.balanceOf(uniswapV2Pair);
    ...
    function setERCTokenAddress(IERC20 _ERC20TokenAddress) public onlyOwner {
        ERC20TokenAddress = _ERC20TokenAddress;
    }

    function setNFTPRice(uint256 _cost) public onlyOwner {
        cost = _cost * 10 ** 15;
    }

    function setLPPairAddress(address _uniswapV2Pair) public onlyOwner {
